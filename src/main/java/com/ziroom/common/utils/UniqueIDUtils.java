...
public class UniqueIDUtils {
    ...
    private static Callable<Integer> uidGenerateTask = new Callable<Integer>() {
        ...
        public Integer call() {
            ...
            if (UniqueIDUtils.uniqueIdQueue.size() < UniqueIDUtils.sizeDivide) {
                ...
                setLastStamp(System.currentTimeMillis());
                setLastSize(UniqueIDUtils.uniqueIdQueue.size());
        ...
        private void reDetermineIndicator() {
            ...
            setTotalScale(totalScale + tempScale);
            if (UniqueIDUtils.scaleQueue.size() > 3) {
                setTotalScale(totalScale - UniqueIDUtils.scaleQueue.poll());
            }

            if (UniqueIDUtils.scale != UniqueIDUtils.totalScale / UniqueIDUtils.scaleQueue.size()) {
                setScale(UniqueIDUtils.totalScale / UniqueIDUtils.scaleQueue.size());
                setSizeDivide(UniqueIDUtils.DEFAULT_SIZE_DIVIDE << UniqueIDUtils.scale);
                setSizeCeiling((UniqueIDUtils.DEFAULT_SIZE_CEILING << UniqueIDUtils.scale) + 5);
            }
        }
    };
    ...
    private static void setLastStamp(long stamp) {
        lastStamp = stamp;
    }

    private static void setLastSize(int size) {
        lastSize = size;
    }

    private static void setTotalScale(int tempScale) {
        totalScale = tempScale;
    }

    private static void setScale(int tempScale) {
        scale = tempScale;
    }

    private static void setSizeDivide(int tempSizeDivide) {
        sizeDivide = tempSizeDivide;
    }

    private static void setSizeCeiling(int tempSizeCeiling) {
        sizeCeiling = tempSizeCeiling;
    }
